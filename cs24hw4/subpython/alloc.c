...
//returns true if the current name and ref is a global variable
...
// mark non-garbage memory slots
void mark(const char * cval, Reference ref) {
    (void) ref; //avoid unused arg error
    Value * val = (Value *) cval;

    //if value is unmarked, mark it as being used
    // prevents infinite loops when encountering cycles
    printf("mark for this is : %d\n", val->marked);
    if (val->marked == 0) {
        val -> marked = 1;
        val -> nextLoc = val; //we'll want to "move"
                //marked values to their original location

        //mark children of list nodes and dict nodes recursively
        switch (val->type) {
            case VAL_LIST_NODE: {
                ListValue *lv = (ListValue *) val;
                ListValue *nlv = (ListValue *) deref(lv->list_node.next);
                mark((char *) nlv, nlv -> ref);    
                break;
            }

            case VAL_DICT_NODE: {
                DictValue *dv = (DictValue *) val;
                DictValue *ndv = (DictValue *) deref(dv->dict_node.next);
                mark((char *) ndv, ndv -> ref);
                break;
...
// find where the blocks should be coalesced to
void* findNewLocation() {
    unsigned char * start = mem;
    unsigned char * old_freeptr = mem;
    //iterate through non-free values
    while (start < freeptr) {
        Value * val = (Value *) start;
        int data_size = val->data_size;
        int value_size = sizeof(Value) + data_size;
        if (val -> nextLoc) {
            //all marked vals should already have nextLoc set
            //set nextLoc for garbage to first available slot

            val -> nextLoc = old_freeptr;

            old_freeptr += value_size;
        }    
        //increment 
        start += value_size;
    }
...
void move() {
    ...
    // first loop through all the Values and
    // move the pointers to where calculated
    //   by findNewLocation()
    unsigned char *curr = mem;
    while (curr < freeptr) {
        Value *curr_value = (Value *) curr;
    ...
    // loop through the Values again, using memmove 
    //   to actually move them to their new locations
    unsigned char * start = mem;
    while (start < freeptr) {
        Value * val = (Value *) start;
        int data_size = val->data_size;
        ...
        if (val -> nextLoc) {   
            // since every non-garbage block should have been
            // marked, every non-garbage block also has
            // nextLoc defined      
            Value * new = val -> nextLoc;
            // we choose memmove over memcpy, since
            // memmove works when source and destination
            // overlap/coincide, whereas memcpy doesn't; 
            // since nextLoc is set to the currently location
            // for marked Values, we should use the safer
            // option
            memmove(new, val, value_size);
...
// mark and compact garbage collector; marks all the globals as non
// garbage and the rest as garbage, and then compacts the globals together, 
// overwriting/freeing the garbage blocks
int collect_garbage(void) {
    ...
    void * new_freeptr = findNewLocation();

    move();
    freeptr = new_freeptr;


    printf("\nWait, I DO know how to identify AND collect garbage...maybe :| \n\n");
