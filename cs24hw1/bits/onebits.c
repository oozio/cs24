...
int count_onebits(unsigned int n) {
  ...
  // This function compares a bitmask m to our number. M is continually incremented, so that it checks a different bit in n at each iteration; if that digit checks out, it must be one, so we can then increment our counter.
