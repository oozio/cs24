...
void __sthread_finish(void) {
    ...
    // Every time we switch contexts, we want to make sure that 
    // we can't be interrupted by the timer function
    // since we want the context switching to be atomic.

    // This means we lock before switching and unlock after. 
    __sthread_lock();
    __sthread_switch();
    __sthread_unlock();
}
