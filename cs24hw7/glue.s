...
__sthread_lock:
        # atomically move 1 into eax
        movl    $1, %eax
        # atomically set scheduler_lock to 1 and 
        # set eax to the previous lock state
        xchg    %eax, scheduler_lock
        # if the previous lock state is 0, return 1
        # if the previous lock state is 1, return 0
        xor     scheduler_lock, %eax
        ret
