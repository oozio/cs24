...
ThreadContext *__sthread_scheduler(ThreadContext *context) {
    ...
    // if current or context is null, we take the first ready thread
    ...
    // if the current thread is finished, delete it
    if (current->state == ThreadFinished) {
        __sthread_delete(current);

    // if the current thread is running, put it in the 
    //   ready queue
    } else if (current-> state == ThreadRunning) {
        current->state = ThreadReady;
        enqueue_thread(current);
    }
    // if the current thread is blocked, put it in the
    // blocked queue 
     else {
        enqueue_thread(current);
    }

    // if both the ready and blocked queues are empty, 
    // all our threads have completed, and we return successfully
    if (queue_empty(&ready_queue)) {
        if (queue_empty(&blocked_queue)) {
            exit(0);
        } 
        // if the blocked queue is not empty, then we've failed somewhere
        else {
            exit(1);
        }
    } 
    // if the ready queue is not empty, take the first thread in the ready
    // list, and run it
    else {
        current = queue_take(&ready_queue);
...
Thread * sthread_create(void (*f)(void *arg), void *arg) {
    // create a new stack with size DEFAULT_STACKSIZE
    char * new_stack = malloc(DEFAULT_STACKSIZE);
 ...
 */
void __sthread_delete(Thread *threadp) {
    // free everything we malloc'd, which were the memory/stack field
