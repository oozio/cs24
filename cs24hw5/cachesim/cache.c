 ...
 */
addr_t get_offset_in_block(cache_t *p_cache, addr_t address) {
    // calculate the offset number of bits
    uint32_t offset_num = p_cache->block_offset_bits;

    // mask for the bottom few bits, which is the offset
    uint32_t offset = address & ((1 << offset_num) - 1);
    return offset;
}
...
void decompose_address(cache_t *p_cache, addr_t address,
    ...
    // use get_offset_in_block for offset
    uint32_t offset_num = p_cache->block_offset_bits;
    *offset = get_offset_in_block(p_cache, address);

    // find set bits, mask for only those bits
    uint32_t set_num = log_2(p_cache->num_sets); 
    *set = (address >> offset_num) & ((1 << set_num) - 1);

    // find tag bits, mask for only those bits
    *tag = (address >> (offset_num + set_num));
...
addr_t get_block_start_from_address(cache_t *p_cache, addr_t address) {
    ...
    // set the offset bits to 0, and return the result
    uint32_t offset_num = p_cache->block_offset_bits;
    addr_t start = address & ~ ((1 << offset_num) - 1);
...
addr_t get_block_start_from_line_info(cache_t *p_cache,
    ...
    // get the number of bits for each component of address
    uint32_t offset_num = p_cache->block_offset_bits; 
    uint32_t set_num = log_2(p_cache->num_sets); 
    uint64_t tag_num = offset_num + set_num;

    // put the tag and set number together in the appropriate order,
    // with an appropriate amount of space for each, and an 0 offset
    addr_t start = (tag << tag_num) | set_no << offset_num;
    return start;
}
...
cacheline_t * find_line_in_set(cacheset_t *p_set, addr_t tag) {
    ...
    cacheline_t *curr_line;
    for (int line_no = 0; line_no < p_set->num_lines; line_no ++) {
        // loop through cache lines in this set 
        curr_line = p_set->cache_lines + line_no;

        // if the current line satisfies our requirements, return it
	    if ((curr_line->tag == tag) && (curr_line->valid == 1)) {
...
cacheline_t * choose_victim(cacheset_t *p_set) {
    ...
    // generate a large(recent) time as a reference for the access times
    uint64_t minTime = clock_tick();
    cacheline_t *curr_line;
    ...
    for (int line_no = 0; line_no < p_set->num_lines; line_no ++) {
        ...
        // if this line is invalid, return it
        if (!(curr_line->valid)) {
            victim = curr_line;
            break;
        } 
        // otherwise, look for the longest-ago access time cache line
        if (curr_line->time < minTime) {
            minTime = curr_line->time;
            victim = curr_line;
        }
    }
